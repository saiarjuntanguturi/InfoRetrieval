...
def create_index(tokens):
    ...
    for i,j in enumerate(tokens):
        for char in j:
            list_of_tuples.append((char,i))
    result = defaultdict(list)
    for k,v in sorted(list_of_tuples):
        result[k].append(v)
    return result
...
def sort_by_num_postings(words, index):
    ...
    result = {}
    for each in words:
        result[each] = len(index[each])
    resulting_tuples = sorted(result.items(), key = lambda (k,v) : v)
    resulting_list = []
    for k,v in resulting_tuples:
        resulting_list.append(k)
...
def search(index, query):
    ...
    [1]
    """
    list1 = sort_by_num_postings(tokenize(query),index)
    map = index[list1[0]]
    for i in range(1,len(list1)):
        map = intersect(map,index[list1[i]])
    return map
